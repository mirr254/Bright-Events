...
def add_event(logged_in_user): 
    ...
    #check for dates
    user_input_date = datetime.strptime( request.json.get('date'), "%yyyy/%m/%d %H:%M")
    ...
    if event:
        user_input_date = datetime.strptime( request.json.get('date'), "%yyyy/%m/%d %H:%M")
        event_stored_date = datetime.strptime(event.date, "%yyyy/%m/%d %H:%M")           
        if ( event_stored_date.strftime("%yyyy/%m/%d %H:%M") == user_input_date.strftime("%yyyy/%m/%d %H:%M") ):                 
